...
public:
    ...
    Entity(GLuint texture_id, float speed, glm::vec3 acceleration, float jump_power,
           int walking[4][3], float animation_time,
    ...
    Entity(GLuint texture_id, float speed, float width, float height, EntityType entity_type,
           AIType ai_type, AIState ai_state, bool flip_by_dir);
    ...
    //health mechanism for enemies
    void  set_health(int hp)   noexcept { m_health = hp; }
    int   health()       const noexcept { return m_health; }
    bool  take_hit(int dmg = 1) noexcept    // returns true if killed
    {
        m_health -= dmg;
        if (m_health <= 0) deactivate();
        return m_health <= 0;
    }
    ...
    int        const get_facing_dir()     const { return m_facing_dir; }
